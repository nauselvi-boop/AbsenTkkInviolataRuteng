          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Menampilkan zona geofence sekolah ({geofenceConfig.radiusMeters}m) dan koordinat absensi seluruh guru & staf hari ini.
              </p>
            </div>
        ...
        <div className="lg:col-span-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[11px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
              {todayRecords.length} Kehadiran
            </span>
          </div>
          <div className="flex-1 overflow-y-auto space-y-3 max-h-[460px] pr-1">
            {todayRecords.length === 0 ? (
              <div className="text-center py-12 text-slate-400 text-xs">
                Belum ada guru atau pegawai yang melakukan absensi hari ini.
              </div>
            ) : (
  ...
  const renderIzin = () => (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      ...
      {izinLoading ? (
        ...
        <p className="text-center text-slate-400">Tidak ada permintaan izin.</p>
      ) : (
            ...
            <tbody>
              {izinRequests.map((req) => (
                <tr key={req.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${req.type === 'dispensasi' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-1.5 p-1.5 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-x-auto">
        {[
          ...
          { id: 'izin', icon: <CheckSquare className="w-4 h-4" />, label: 'Izin & Kunci' },
